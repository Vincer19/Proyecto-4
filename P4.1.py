...
# ---
# 
# * Estudiante: **Vincer Solis Hernandez**
# * Carné: **B87702**
# * Grupo: **1/2**
# 
# ---
...
def modulador_I(bits, fc, mpp):
    ...
    return senal_Tx_I, Pm1, portadora_I, moduladora_I  
...
def modulador_Q(bits, fc, mpp):
    ...
    moduladora = moduladora_Q + moduladora_I 
    return senal_Tx_Q, Pm2, portadora_Q, moduladora_Q, Pm, senal_Tx,  portadora, moduladora
...
senal_Tx_I, Pm1, portadora_I, moduladora_I = modulador_I(bits_Tx, fc, mpp)

senal_Tx_Q, Pm2, portadora_Q, moduladora_Q, Pm, senal_Tx, portadora, moduladora = modulador_Q(bits_Tx, fc, mpp)
...
print('{} errores, para un QPSK de {:0.4f}.'.format(errores, BER))
...
# la modulación QPSK se puede ver en las funciones definidas arriba 
...
A = [1,-1]  
# Se tiene dos posibles valores para la amplitud    
N = 4       
# Se trabajará con 4 posibilidades de la onda    
X_t = np.empty((N, len(t)))
# N funciones del tiempo x(t) con T puntos      
...
realiz, = plt.plot(t, P, lw=6, color = 'blue', label='Promedio Realizaciones')      
 # Graficar el resultado teórico del valor esperado     
E = np.mean(senal_Tx)*t     
teorico, = plt.plot(t, E, '-.', lw=3, color = 'r', label='Valor teorico')      
# Mostrar las realizaciones, y su promedio calculado y teórico     plt.title('Realizaciones del proceso aleatorio $X(t)$')     plt.xlabel('$t$')     plt.ylabel('$x_i(t)$')     
...
# Con este grafico se puede comprobar que la señal tx cumple la estacionaridad ya que el promedio teorico y el de las realizaciones 
# dieron iguales a 0 por lo que cumple lo de ser constante para que se cumpla la estacionaridad y la ergodicidad se cumple ya que tanto el promedio
# temporal y estadistico dio igual, como se puede ver en la grafica donde la linea azul es el de las realizaciones y la roja el teorico.
...
Nm = len(senal_Tx)
# Numero de simbolos es 1/2 * mpp ya que con la formula de M =2^k se obtiene que tienen que ser para QPSK 2 bits por simbolo 
# lo cual por la naturalida de mpp seria 1/2 del mpp de la entrada de una senal
Ns = Nm // (mpp/2)
# Tiempo del simbolo = periodo de la onda periodica
Tc = 1 / fc
